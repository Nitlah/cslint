...
type subCertHasDisallowEKUs struct{}
...
func init() {
	lint.RegisterLint(&lint.Lint{
		Name: "e_cert_eku_has_disallow_eku",
		Description: "If the Certificate is a Timestamp Certificate, then id-kp-timeStamping MUST be present and MUST be marked critical." +
			"Additionally, the following EKUs MUST NOT be present:" +
			"• anyExtendedKeyUsage" +
			"• id-kp-serverAuth",
...
func NewSubCertHasDisallowEKUs() lint.LintInterface {
	return &subCertHasDisallowEKUs{}
}

func (l *subCertHasDisallowEKUs) CheckApplies(c *x509.Certificate) bool {
	return util.IsSubscriberCert(c)
}

func (l *subCertHasDisallowEKUs) Execute(c *x509.Certificate) *lint.LintResult {
	for _, v := range c.ExtKeyUsage {
		if v == x509.ExtKeyUsageAny || v == x509.ExtKeyUsageServerAuth {
