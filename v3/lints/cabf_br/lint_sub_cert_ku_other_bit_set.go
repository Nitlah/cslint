...
type subCertKUOtherBitSet struct{}
...
func init() {
	lint.RegisterLint(&lint.Lint{
		Name:          "e_sub_cert_ku_other_bit_set",
		...
		Lint:          NewSubCertKUOtherBitSet,
	})
}

func NewSubCertKUOtherBitSet() lint.LintInterface {
	return &subCertKUOtherBitSet{}
}

func (l *subCertKUOtherBitSet) CheckApplies(c *x509.Certificate) bool {
	return util.IsSubscriberCert(c) && util.IsExtInCert(c, util.KeyUsageOID)
}

func (l *subCertKUOtherBitSet) Execute(c *x509.Certificate) *lint.LintResult {
	ku := c.KeyUsage
