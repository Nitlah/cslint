...
type subCaHasDisallowEKUs struct{}
...
func init() {
	lint.RegisterLint(&lint.Lint{
		Name: "e_sub_ca_eku_has_disallow_eku",
...
func NewSubCaHasOtherEKUs() lint.LintInterface {
	return &subCaHasDisallowEKUs{}
}

func (l *subCaHasDisallowEKUs) CheckApplies(c *x509.Certificate) bool {
...
func (l *subCaHasDisallowEKUs) Execute(c *x509.Certificate) *lint.LintResult {
	...
	setEKU[int(x509.ExtKeyUsageEmailProtection)] = 1

	for _, v := range c.ExtKeyUsage {
		if _, exists := setEKU[int(v)]; !exists {
